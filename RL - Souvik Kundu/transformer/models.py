...
#        self.pos_emb = PosEncoding(25, 300) # TODO: *10 fix
        ...
        self.pos_emb= nn.Embedding(240, 300, padding_idx=0)
        self.pos_emb.weight.data = position_encoding_init(240, 300)
...
#                               opt.max_tgt_seq_len, opt.tgt_vocab_size, opt.dropout, opt.weighted_model)
        self.encoder = Encoder_Decoder(opt.n_layers, opt.d_k, opt.d_v, opt.d_model, opt.d_ff, opt.n_heads,
                       opt.max_tgt_seq_len, opt.tgt_vocab_size, opt.dropout, opt.weighted_model)
...
#            self.encoder.embedding_table.weight = self.decoder.tgt_emb.weight
...
#        enc_embeddings = set(map(id, self.encoder.embedding_table.parameters()))
        dec_freezed_param_ids = set(map(id, self.encoder.pos_emb.parameters()))
#        freezed_param_ids = enc_freezed_param_ids | dec_freezed_param_ids
    ...
    def ed(self, dec_inputs, dec_inputs_len, mask_dec, enc_inputs, enc_inputs_len, mask_enc):
        return self.encoder(dec_inputs, dec_inputs_len, mask_dec, enc_inputs, enc_inputs_len, mask_enc)


    def forward(self, enc_inputs, enc_inputs_len, dec_inputs, dec_inputs_len, return_attn=False):
#        print(enc_inputs.size(), enc_inputs_len.size(), dec_inputs.size(), dec_inputs_len.size())
#        print(enc_inputs, enc_inputs_len, dec_inputs, dec_inputs_len)
#        asas
